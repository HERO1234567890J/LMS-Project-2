...
// دالة تشغيل التسلسل الهرمي (فرقة -> مدرس -> مادة)
function initHierarchyFilters() {
  const stageSelect = document.getElementById('acStageSelect');
  const teacherSelect = document.getElementById('acTeacherSelect');
  const courseSelect = document.getElementById('acCourseSelect');

  if (stageSelect && teacherSelect && courseSelect) {
    // لما تختار الفرقة، يفتحلك المدرسين
    ...
    // لما تختار المدرس، يجيبلك مواده (مؤقتاً بيانات وهمية لحد ما نربط بالباك إند)
    teacherSelect.addEventListener('change', async () => {
      if (teacherSelect.value) {
        courseSelect.disabled = false;
        courseSelect.innerHTML = '<option value="">جارِ جلب المواد...</option>';

        // ده محاكاة لحد ما نربط السيرفر
        setTimeout(() => {
          courseSelect.innerHTML = `
            <option value="">-- اختر المادة --</option>
            <option value="1">القانون المدني</option>
            <option value="2">تاريخ القانون</option>
          `;
        }, 500);
      } else {
        courseSelect.disabled = true;
...
async function submitAddContent() {
  ...
  try {
    ...
    if (files.length) {
      ...
      await api(`/lessons/${lessonId}/attachments`, { method: 'POST', body: fd });
    }

    const publishNow = document.getElementById('acPublishNow')?.checked;
    if (!publishAt && publishNow) {
      await api(`/lessons/${lessonId}/status`, { method: 'PATCH', body: JSON.stringify({ status: 'published' }) });
    }
  ...
  try {
    ...
    container.querySelectorAll('[data-grade-answer]').forEach((btn) =>
      btn.addEventListener('click', async () => {
        ...
        btn.disabled = true;
        try {
          await api(`/answers/${btn.dataset.gradeAnswer}/grade`, { method: 'POST', body: JSON.stringify({ points }) });
...
window.toggleCourseVisibility = async function (id, currentStatus) {
  ...
  try {
    await api(`/courses/${id}/status`, { method: 'PATCH', body: JSON.stringify({ status: newStatus }) });
    ...
    courseForm.onsubmit = async (e) => {
      ...
      try {
        ...
        if (currentEditCourseId) {
          res = await api(`/courses/${currentEditCourseId}`, { method: 'PUT', body: JSON.stringify(payload) });
...
// ==================== قسم المبيعات والأرباح (النظام المتقدم) ====================
async function loadSalesData() {
  const teacherFilterEl = document.getElementById('salesTeacherFilter');
  const dateFromEl = document.getElementById('salesDateFrom');
  const dateToEl = document.getElementById('salesDateTo');
  const tableBody = document.getElementById('salesTableBody');
  const payoutBox = document.getElementById('payoutActionBox');

  if (!tableBody) return;

  const teacherFilter = teacherFilterEl ? teacherFilterEl.value : 'all';
  const dateFrom = dateFromEl ? dateFromEl.value : '';
  const dateTo = dateToEl ? dateToEl.value : '';

  if (payoutBox) {
    payoutBox.style.display = (teacherFilter !== 'all') ? 'block' : 'none';
  }

  tableBody.innerHTML = '<tr><td colspan="6" class="table-empty">جارِ تحميل المبيعات...</td></tr>';

  setTimeout(() => {
    // ضفتلك عملية بيع بتاريخ "النهارده" أوتوماتيك عشان تجرب عليها براحتك
    const todayDate = new Date().toISOString().split('T')[0];

    const dummySales = [
      { id: 'FWRY-98237', student: 'يوسف أحمد', lecture: 'القانون المدني 1', price: 50, is_paid: false, date: '2026-08-20', teacher_id: '1' },
      { id: 'FWRY-98238', student: 'محمد علي', lecture: 'تاريخ القانون 3', price: 60, is_paid: true, date: '2026-08-21', teacher_id: '2' },
      { id: 'FWRY-98239', student: 'سارة خالد', lecture: 'القانون المدني 2', price: 50, is_paid: false, date: '2026-08-22', teacher_id: '1' },
      { id: 'FWRY-98240', student: 'أحمد حسن', lecture: 'القانون الجنائي', price: 70, is_paid: false, date: '2026-08-25', teacher_id: '3' },
      { id: 'FWRY-99999', student: 'طالب تجريبي', lecture: 'محاضرة اليوم', price: 100, is_paid: false, date: todayDate, teacher_id: '1' }
    ];

    // تحديث أرباح المنصة الكلية (عشان متفضلش صفر وتفتكرها باظت!)
    const totalPlatform = dummySales.reduce((sum, s) => sum + s.price, 0);
    const tpEl = document.getElementById('totalPlatformRevenue');
    if (tpEl) tpEl.textContent = totalPlatform;

    let filtered = dummySales;
    ...
    const pendingRev = filtered.filter(s => !s.is_paid).reduce((sum, s) => sum + s.price, 0);
    const paidRev = filtered.filter(s => s.is_paid).reduce((sum, s) => sum + s.price, 0);

    const pendingEl = document.getElementById('teacherPendingRevenue');
    const paidEl = document.getElementById('teacherPaidRevenue');
    const countEl = document.getElementById('teacherFilteredLectures');

    if (pendingEl) pendingEl.textContent = pendingRev;
    if (paidEl) paidEl.textContent = paidRev;
    if (countEl) countEl.textContent = filtered.length;

    if (filtered.length === 0) {
      tableBody.innerHTML = '<tr><td colspan="6" class="table-empty">لا توجد مبيعات في هذا التاريخ لهذا المدرس.</td></tr>';
    } else {
      tableBody.innerHTML = filtered.map(s => `
        <tr>
          ...
          <td>${esc(s.student)}</td>
          <td>${esc(s.lecture)}</td>
          <td><strong>${s.price} ج.م</strong></td>
          <td>
            ${s.is_paid ? '<span class="badge badge-success">تم تسويتها</span>' : '<span class="badge badge-warning">معلقة للمدرس</span>'}
          </td>
          <td>${s.date}</td>
        </tr>
      `).join('');
...
function initSales() {
  ['salesTeacherFilter', 'salesDateFrom', 'salesDateTo'].forEach(id => {
    const el = document.getElementById(id);
    if (el) {
      el.addEventListener('change', loadSalesData);
    }
  });

  const btn = document.getElementById('settlePayoutBtn');
  if (btn) {
    btn.addEventListener('click', () => {
      const teacherSelect = document.getElementById('salesTeacherFilter');
      const teacherName = teacherSelect.options[teacherSelect.selectedIndex].text;
      const pendingAmount = document.getElementById('teacherPendingRevenue').textContent;

      if (pendingAmount == 0) return toast('لا توجد أرباح معلقة لتسويتها.', 'error');

      if (confirm('هل أنت متأكد من تسديد مبلغ ' + pendingAmount + ' ج.م لـ ' + teacherName + ' وتصفير العداد؟')) {
        toast('تم تصفير العداد وتسجيل الدفعة بنجاح!');
      }
    });
  }
