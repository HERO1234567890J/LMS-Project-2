(function () {
  ...
  async function loadAvailableCourses() {
    const grid = $('availableCoursesGrid');
    try {
...
window.buyLecture = function(id, price, title) {
  const walletEl = document.getElementById('walletBalance');
  const currentBalance = parseInt(walletEl ? walletEl.textContent : 0);
  
  if(confirm(`هل تريد شراء "${title}" بـ ${price} ج.م من رصيد محفظتك؟`)) {
    if(currentBalance >= price) {
      // خصم الفلوس من المحفظة في الواجهة
      if (walletEl) walletEl.textContent = currentBalance - price;
      alert('🎉 تم شراء المحاضرة بنجاح!');
      
      // ملاحظة: هنا سيتم وضع الـ API الذي يبلغ الباك إند بالشراء لاحقاً
      // await api('/buy-lesson', { method: 'POST', body: JSON.stringify({ lesson_id: id }) });
      
    } else {
      alert('❌ رصيدك غير كافٍ. برجاء شحن المحفظة أولاً بالضغط على زر "شحن" بالأعلى.');
    }
  }
};
